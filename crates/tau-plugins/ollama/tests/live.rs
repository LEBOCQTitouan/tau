//! Live smoke tests against a local Ollama instance.
//!
//! Setup:
...
//!
//! Override the model with TAU_OLLAMA_LIVE_MODEL=llama3.1, etc.
//!
//! These tests are #[ignore]-by-default. CI does not run them.
//! Maintainer-triggered to detect Ollama API drift between cassette
//! re-records.
...
/// Build an `OllamaConfig` for live tests. Defaults to local Ollama
/// at <http://localhost:11434> with no bearer token.
fn live_config() -> OllamaConfig {
    ...
    // mutate. Default base_url is already http://localhost:11434.
...
async fn live_complete_smoke() {
    if std::env::var("TAU_OLLAMA_LIVE_TESTS").is_err() {
        eprintln!("skipping: TAU_OLLAMA_LIVE_TESTS not set");
        return;
    }
    let plugin = OllamaPlugin::from_config(live_config()).unwrap();
