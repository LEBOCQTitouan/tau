...
"#;
    ...
    assert_eq!(
        pre.schema_version, 2,
        "load should auto-upgrade v1 to v2 in memory",
    );
    ...
    // lockfile-write path, which should persist `schema_version = 2`.
    ...
    assert!(
        updated.contains("schema_version = 2"),
        "lockfile on disk should be schema_version = 2 after auto-upgrade install; got:\n{updated}",
    );
    assert!(
