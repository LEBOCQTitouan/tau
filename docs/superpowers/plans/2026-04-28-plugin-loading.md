...
- **`tau-ports` has no `serde` feature** (discovered in Task 1). The plan's Cargo.toml templates initially prescribed `tau-ports = { workspace = true, features = ["serde"] }` for `tau-plugin-protocol` and `tau-plugin-sdk`, but `tau-ports/Cargo.toml` exposes only `default` and `test-fixtures` features. Task 1 dropped the non-existent feature flag from both new crates' `tau-ports` deps. **The first task that needs serde derives on `tau-ports` types crossing the wire (likely Task 5 handshake payloads or Task 9 LLM/Tool runners)** must (a) add a `serde` feature to `tau-ports/Cargo.toml` propagating to `tau-domain/serde`, and (b) restore `features = ["serde"]` on the two new crates' `tau-ports` deps.
