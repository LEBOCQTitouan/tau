...
| **Container-adapter HTTP plugin tests fail with bridge wrapping.** Sub-project H (PR <TBD>) wired the proxy bind-mount + `tau-net-bridge` wrap on the Container adapter and authored the 3 cassette tests (anthropic / ollama / openai), but they fail in CI with `PluginHandshakeFailed: EOF before handshake response`. The strict-tier proxy works (`strict_proxy.rs` integration tests pass on stock Linux without privileges), so the bug is in the Container-adapter wrapping path specifically, not the proxy itself. Tests stay `#[ignore]`'d pending interactive Linux debug. **Also:** sub-project D's 2 earlier shell/fs-read Container tests (#[ignore]'d for the same root cause: host-path binary not existing inside container). Sub-project I (per-plugin images, PR <TBD>) **partially closes** the row: D's 2 fs/shell tests un-`#[ignore]`'d and passing in CI; the 3 H HTTP cassette tests remain `#[ignore]`'d but with the per-plugin-image foundation, plain-HTTP proxy support, and `--add-host=host.docker.internal:host-gateway` wiring all in place. The HTTP tests pass locally on macOS Apple Silicon Podman (via slirp4netns rootless networking — for the wrong reason; bypasses the proxy entirely) but fail in CI on Linux Docker `--network bridge` despite the host-gateway fix. **Sub-project J** needs an interactive Linux Docker session to debug why `host.docker.internal` doesn't reach the cassette server. **D-half closed by sub-project I on 2026-05-08; H-half awaits sub-project J.** | **H follow-up** | Bridge wrapping inside Docker container fails before plugin handshake. Possible causes: container-image libc mismatch with the host-built bridge binary; bridge fork/exec path issue inside Docker; docker volume-mount of the bridge binary not landing where expected. Needs `docker run -it ... bash` to reproduce + bridge stderr capture. |
