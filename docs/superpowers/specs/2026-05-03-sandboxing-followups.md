...
| **Container-adapter network filtering is unimplemented.** The 3 `layer4_container.rs` HTTP plugin tests (anthropic / ollama / openai cassette-replay) remain `#[ignore]`'d. The Container adapter inherits the trait's no-op `apply_post_spawn`; F's veth IP `10.222.1.1` is a NativeSandbox construct, and inside a Docker container the host is not reachable at that IP. A future sub-project (or extension to the Container adapter) needs to plumb network isolation differently — Docker host networking, `--add-host`, a sidecar proxy, or per-container iptables. | **F follow-up** (new sub-project) | Container adapter uses Docker; networking semantics fundamentally differ from netns+veth. Requires its own design pass. |
| **strict_net_filter.rs integration tests hang in CI.** All 4 tests in `crates/tau-sandbox-native/tests/strict_net_filter.rs` are `#[ignore]`'d. When `test-net-filter / linux` (privileged Docker, full root + CAP_NET_ADMIN) actually ran them, all 4 hung past 60s and the run had to be cancelled. Notably `no_network_cap_socket_denied_by_seccomp` also hangs — it has no `Network(Http)` in plan and no `apply_post_spawn` runs, suggesting the hang is in the seccomp `KillProcess` / `cmd.output()` propagation path on Linux, NOT in F-specific code. The lib unit tests for `net_filter` (probe / validate / resolve / netns / rules / handle) all pass. | **F task 6.5 follow-up** | Needs a real-Linux debugging session to repro the hang and trace where the parent process gets stuck after the child receives SIGSYS. Cannot be reproduced on darwin. |
