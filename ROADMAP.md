...
| 12-F | Per-host network filtering — machinery (PARTIAL) ✅ | Sub-project F from the priority-12 followups — see [spec](docs/superpowers/specs/2026-05-06-sandbox-net-filter-design.md), [plan](docs/superpowers/plans/2026-05-06-sandbox-net-filter.md), and [ADR-0019](docs/decisions/0019-per-host-network-filter.md). New `tau-sandbox-native::net_filter` module (~640 LOC, 26+ unit tests) implements probe + validate + DNS resolve + veth/netns setup + nft ruleset generation/apply + NetFilterHandle + orchestrator (`apply_per_host_filter`). New `SandboxError::NetFilter { message: String }` variant in `tau-ports`. **Strict.rs integration deferred to F task 6.5** — `Sandbox::wrap_spawn` returns before child is spawned, but per-host filter setup needs the child PID; clean solution is a new `Sandbox::apply_post_spawn` trait method. Until task 6.5 lands, `unshare_flags_for_plan` retains its v0.1 fallback (drops CLONE_NEWNET when Network(Http) is in plan; child inherits parent netns). The 3 `#[ignore]`'d Layer 4 container × HTTP plugin tests stay ignored. CI infrastructure: GHA host runners block uid_map writes (Phase 0 finding); the new `test-net-filter / linux` job runs inside privileged Docker (`docker run --privileged rust:1.95-bookworm`). 15 required CI checks gating `main` (was 14). | 2026-05-06 | 2026-05-06 |
