...
| 12-F | Per-host network filtering — Native adapter integration ✅ | Sub-project F from the priority-12 followups — see [spec](docs/superpowers/specs/2026-05-06-sandbox-net-filter-design.md), [task 6.5 spec](docs/superpowers/specs/2026-05-06-net-filter-integration-design.md), [task 6.5 plan](docs/superpowers/plans/2026-05-06-net-filter-integration.md), and [ADR-0019](docs/decisions/0019-per-host-network-filter.md). Phase 1 (PR #35, commit d4438ae): `tau-sandbox-native::net_filter` machinery (~640 LOC, 26+ unit tests) — probe + validate + DNS resolve + veth/netns + nft ruleset + `NetFilterHandle` + orchestrator. Phase 2 / task 6.5 (PR #37): `Sandbox::apply_post_spawn` trait extension + sync-pipe rendezvous in `apply_strict` + `NativeSandbox` veth_subnets HashMap + `validate_plan` hard-refuse on F-unavailable hosts + runtime caller + `unshare_flags_for_plan` flip (always `CLONE_NEWUSER \| CLONE_NEWNET`). The Native adapter is fully integrated. Two follow-ups carried over: (1) Container-adapter network filtering (the 3 `layer4_container.rs` HTTP plugin tests remain `#[ignore]`'d — Docker has its own networking; F's veth IP is a NativeSandbox construct), and (2) `strict_net_filter.rs` integration test hang in CI (all 4 tests `#[ignore]`'d; runs in privileged Docker hang past 60s including the no-Network(Http) test, suggesting a seccomp/cmd.output() propagation issue, not F-specific code). 15 required CI checks gating `main`. | 2026-05-06 | 2026-05-06 |
